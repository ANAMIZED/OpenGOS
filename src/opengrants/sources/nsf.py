...
logger = logging.getLogger("opengrants.sources.nsf")
...
class NSFClient:
    ...
    async def search(self, keyword: str, rows: int = 25) -> list[NSFAward]:
        params = {
            "keyword": keyword,
            "printFields": "id,title,abstractText,startDate,expDate,estimatedTotalAmt,piFirstName,piLastName,awardeeName",
            "rpp": min(rows, 100),
        }
        try:
            ...
            try:
                ...
                awards.append(
                    NSFAward(
                        ...
                        amount=float(item["estimatedTotalAmt"]) if item.get("estimatedTotalAmt") else None,
                        pi=f"{item.get('piFirstName', '')} {item.get('piLastName', '')}".strip() or None,
                        organization=item.get("awardeeName"),
                        source_url=f"https://www.nsf.gov/awardsearch/showAward?AWD_ID={award_id}" if award_id else None,
                        raw=item,
                    )
                )
            except Exception:
