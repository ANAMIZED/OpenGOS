...
logger = logging.getLogger("opengrants.grants_client")
...
class GrantsClient:
    ...
    def __init__(self, timeout: float = 30.0, user_agent: str = "OpenGrants-OS/0.1"):
    ...
    async def search(
        ...
        """
        Search the public Grants.gov API.

        No API key required for basic keyword search.
        """
    ...
    def _normalize(self, item: dict[str, Any]) -> GrantOpportunity | None:
        """Normalize heterogeneous response shapes into GrantOpportunity."""
        opp_id = str(
            item.get("id")
        ...
        number = (
            item.get("number")
            or item.get("opportunityNumber")
            or item.get("oppNumber")
        )
        description = (
