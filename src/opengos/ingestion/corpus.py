...
logger = logging.getLogger("opengos.ingestion")
...
class CorpusManager:
    ...
    async def refresh(
        self, keywords: list[str] | None = None, max_per_keyword: int = 20
    ) -> int:
    ...
    def stats(self) -> dict[str, Any]:
        return {
            ...
            "open_source_relevant": sum(
                1 for x in self._cache.values() if x.get("open_source_relevant")
            ),
...
async def refresh() -> dict[str, Any]:
    """MCP-facing corpus refresh helper."""
    global _manager
    if _manager is None:
        _manager = CorpusManager()
    added = await _manager.refresh()
